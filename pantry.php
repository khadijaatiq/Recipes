...
(SELECT 
    ...
    ROUND(
        COUNT(DISTINCT CASE WHEN ui.ingredient_id IS NOT NULL THEN ri.ingredient_id END) * 100.0 / COUNT(DISTINCT ri.ingredient_id),
        1
    ) AS match_percentage
FROM recipes r
...
FROM user_submitted_recipes u
JOIN recipe_ingredients ri ON u.recipe_id = ri.recipe_id
LEFT JOIN user_ingredients ui 
    ON ui.ingredient_id = ri.ingredient_id 
    AND ui.user_id = ? 
LEFT JOIN ratings rt ON u.recipe_id = rt.recipe_id
WHERE u.status = 'public'
GROUP BY u.recipe_id, u.name, u.image_url)

ORDER BY match_percentage DESC
"; 
$fmi = $conn->prepare($query);
$fmi->bind_param("ii", $user_id, $user_id);
$fmi->execute();
                ...
                <div class="recipe-matches" id="recipe-matches">
                    <div class="recipes-grid" id="recipe-results">
                        <?php while($row = $matching->fetch_assoc()): ?>
                            <?php if ($row['match_percentage'] >= $threshold): ?>
